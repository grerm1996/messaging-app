const { Messages, Convos } = require("../models")

const getMessages = async (req, res) => {
    try {
        const messages = await Messages.find( { "convoId": req.params.convoId } ).sort({ date: -1 });
        const convoObj = await Convos.findOne( { "convoId": req.params.convoId } );
        console.log('convoObj:', convoObj);
        return res.status(200).json({messages, convoObj});
    } catch (error) {
        console.error(error);
